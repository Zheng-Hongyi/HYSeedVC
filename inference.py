...
# 在部分 macOS / MPS 环境下 fp16 或算子可能不稳定；可设 SEED_VC_FORCE_CPU=1 强制走 CPU（更慢但更稳）。
_force_cpu = os.environ.get("SEED_VC_FORCE_CPU", "").lower() in ("1", "true", "yes")
if _force_cpu:
...
def _release_torch_accelerator():
    """正常/中断退出时尽量释放 CUDA/MPS 显存，减轻「断开后下次运行卡顿」。"""
    gc.collect()
    try:
...
def main(args):
    ...
    if f0_condition:
        ...
        # RMVPE 返回 float64 numpy；MPS 不支持 float64，须先 cast 再 .to(device)
        F0_ori = torch.as_tensor(F0_ori, dtype=torch.float32, device=device).unsqueeze(0)
        F0_alt = torch.as_tensor(F0_alt, dtype=torch.float32, device=device).unsqueeze(0)
        ...
        if args.f0_scale != 1.0:
            shifted_f0_alt[F0_alt > 1] = shifted_f0_alt[F0_alt > 1] * args.f0_scale
    else:
        F0_ori = None
        F0_alt = None
        shifted_f0_alt = None
    ...
    # TorchAudio 2.9+ 默认走 torchcodec，未安装会报错；改用 soundfile 写 wav
    wav_np = np.clip(vc_wave.cpu().squeeze(0).numpy(), -1.0, 1.0)
    sf.write(out_wav, wav_np, int(sr), subtype="PCM_16")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="仅当 --f0-condition True 时生效：在有声段将转换侧 F0 乘以该系数（例如 0.75 压低音高）",
    )
